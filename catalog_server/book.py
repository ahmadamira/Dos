...
class Book(db.Model):
    ...
    def search(cls, topic):
        ...
        return cls.query.filter(cls.topic.ilike(f'%{topic}%'))
...
database_init += [
    ...
    Book('How to finish Project 3 on time', 'Projest mangment ', 23, 18.00),
    Book('Why theory classes are so hard', 'Theory classes', 12, 20.00),
    Book('Spring in the Pioneer Valley', 'Litreture', 30, 11.00),
]
...
# Define Marshmallow Formatter Schema class for Dump.
